...
1. **Navbar** – logo, 4+ routes, login/register, sticky, responsive  
   * After login, show a dropdown with:  
     * Logged-in user info (replacing login/register)  
     * Add Product  
     * Manage Products

2. **Hero** – headline, subtitle, primary CTA, optional background  
3. **4 Relevant Sections** – choose based on theme (e.g., features, items, testimonials, banner)  
   * Cards/layout must be uniform with hover/focus states  
   * Clear hierarchy, spacing, and responsive design  
4. **Footer** – links, optional social icons, copyright, consistent spacing
...
1. **Layout & Responsiveness** – consistent spacing, clean layouts, adaptive for mobile/tablet/desktop.  
2. **Typography & Colors** – clear hierarchy, readable fonts, consistent color palette.  
3. **Cards, Lists & Forms** – uniform cards with hover/focus, responsive grids, clean forms with inline validation and optional loading states.  
4. **Interactions & Consistency** – Hover/focus for interactive elements, visual consistency across pages, optional micro-animations.

## **Technologies**

* **Next.js (App Router)** for the frontend application  
* **Firebase** for authentication ([Read Documentation](https://firebase.google.com/codelabs/firebase-nextjs))
